...
import tech.itpark.project_delivery_web.jdbc.JdbcTemplate;
import tech.itpark.project_delivery_web.model.TokenAuth;
...
public class UserRepository {
  ...
  public User save(User user) {
    try (
        ...
        final var statement = connection.prepareStatement("INSERT INTO users(login, password, secret) VALUES (?, ?, ?)",
  ...
  public Optional<User> getByLogin(String login) {
    try (
        final var connection = ds.getConnection();
        final var statement = connection.prepareStatement("SELECT id, login, password, secret, roles FROM users WHERE login = ?");
    ) {
      var index = 0;
  ...
  public void save(TokenAuth auth) {
    try (
      ...
      // language=PostgreSQL
      template.update(conn, "INSERT INTO tokens(userId, token) VALUES (?, ?)", auth.getUserId(), auth.getToken());
    } catch (SQLException e) {
      throw new DataAccessException(e);
    }
  }
  ...
  public Optional<User> getByToken(String token) {
    ...
    try (
        final var connection = ds.getConnection();
        final var statement = connection.prepareStatement(
                "SELECT u.id, u.login, '**MASKED**' AS password, '**MASKED**' AS secret u.roles FROM users u JOIN tokens t ON u.id = t.userid WHERE t.token = ?"
            );
    ) {
      var index = 0;
